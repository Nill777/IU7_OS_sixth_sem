#include <sys/socket.h>
#include <stdlib.h>
#include <stdio.h>
#include <unistd.h>
#include <signal.h>
#include <netinet/in.h>
#include <sys/shm.h>
#include <sys/sem.h>
#include <sys/wait.h>
#include <errno.h>

#include "common.h"

int sfd, semid, shmid;
char *arr;

#define READER_QUEUE 0
#define WRITER_QUEUE 1
#define READER 2
#define WRITER 3
#define SHADOW_WRITER 4

struct sembuf start_rd[] = {
        {READER_QUEUE, 1, 0},
        {WRITER_QUEUE, 0, 0},
        {SHADOW_WRITER, 0, 0},
        {READER, 1, 0},
        {READER_QUEUE, -1, 0}
};

struct sembuf stop_rd[] = {
        {READER, -1, 0}
};

struct sembuf start_wr[] = {
        {WRITER_QUEUE, 1, 0},
        {READER, 0, 0},
        {WRITER, -1, 0},
        {SHADOW_WRITER, 1, 0},
        {WRITER_QUEUE, -1, 0}
};

struct sembuf stop_wr[] = {
        {SHADOW_WRITER, -1, 0},
        {WRITER, 1, 0}
};

void process_client(int connfd)
{
    unsigned index;
    char buf[BUF_SIZE];
    int status;
    int full;

    printf("%d Got new connection\n", getpid());

    while (1)
    {
        if (recv(connfd, buf, BUF_SIZE, 0) <= 0)
        {
            printf("%d Client disconnected, closing connection\n", getpid());
            break;
        }

        switch (buf[0])
        {
            case 'r':
                if (semop(semid, start_rd, 5) == -1)
                {
                    perror("semop start_rd");
                    exit(1);
                }
                for (size_t i = 0; i < ARR_SIZE; i++)
                {
                    buf[i] = arr[i];
                }
                if (send(connfd, buf, sizeof(buf), 0) == -1)
                {
                    perror("send");
                    exit(1);
                }
                if (semop(semid, stop_rd, 1) == -1)
                {
                    perror("semop stop_rd");
                    exit(1);
                }
                break;

            case 'w':
                index = (unsigned) buf[1];
                if (semop(semid, start_wr, 5) == -1)
                {
                    perror("semop start_wr");
                    exit(1);
                }
                if (index < ARR_SIZE && arr[index] != ' ')
                {
                    arr[index] = ' ';
                    status = OK;
                    printf("%d Client reserved letter %u\n", getpid(), index);
                }
                else
                {
                    if (index < ARR_SIZE && arr[index] == ' ')
                    {
                        status = ALREADLY_RESERVED;
                        printf("%d Client failed to reserve letter %u (already reserved)\n", getpid(), index);
                    }
                    else
                    {
                        status = ERROR;
                        printf("%d Invalid letter number %u received\n", getpid(), index);
                    }
                }
                full = 1;
                for (size_t i = 0; i < ARR_SIZE; i++)
                {
                    if (arr[i] != ' ')
                    {
                        full = 0;
                        break;
                    }
                }
                if (full)
                {
                    printf("%d All letters reserved. Shutting down server.\n", getpid());
                    kill(getppid(), SIGINT);
                }
                if (send(connfd, &status, sizeof(status), 0) == -1)
                {
                    perror("send");
                    exit(1);
                }
                if (semop(semid, stop_wr, 2) == -1)
                {
                    perror("semop stop_wr");
                    exit(1);
                }
                break;

            default:
                printf("%d Unknown command: %c\n", getpid(), buf[0]);
                break;
        }
    }
}

void sigint_handler(int signum)
{
    close(sfd);
    semctl(semid, 0, IPC_RMID, NULL);
    shmdt(arr);
    shmctl(shmid, IPC_RMID, NULL);
    exit(0);
}

int main(void)
{
    int listenfd, connfd;
    socklen_t clilen;
    struct sockaddr_in cliaddr, servaddr;

    if (signal(SIGINT, sigint_handler) == SIG_ERR)
    {
        perror("cannot set SIGINT handler");
        exit(1);
    }

    signal(SIGCHLD, SIG_IGN);

    semid = semget(IPC_PRIVATE, 5, IPC_CREAT | 0666);
    if (semid == -1)
    {
        perror("semget");
        exit(1);
    }
    if (semctl(semid, READER_QUEUE, SETVAL, 0) == -1)
        perror("semctl READER_QUEUE");
    if (semctl(semid, WRITER_QUEUE, SETVAL, 0) == -1)
        perror("semctl WRITER_QUEUE");
    if (semctl(semid, READER, SETVAL, 0) == -1)
        perror("semctl READER");
    if (semctl(semid, WRITER, SETVAL, 1) == -1)
        perror("semctl WRITER");
    if (semctl(semid, SHADOW_WRITER, SETVAL, 0) == -1)
        perror("semctl SHADOW_WRITER");

    shmid = shmget(IPC_PRIVATE, ARR_SIZE, IPC_CREAT | 0666);
    if (shmid == -1)
    {
        perror("shmget");
        exit(1);
    }
    arr = shmat(shmid, NULL, 0);
    if (arr == (char *)-1)
    {
        perror("shmat");
        exit(1);
    }

    for (size_t i = 0; i < ARR_SIZE; i++)
        arr[i] = 'a' + i;

    listenfd = socket(AF_INET, SOCK_STREAM, 0);
    if (listenfd == -1)
    {
        perror("socket");
        exit(1);
    }
    sfd = listenfd;

    servaddr.sin_family = AF_INET;
    servaddr.sin_addr.s_addr = htonl(INADDR_ANY);
    servaddr.sin_port = htons(SERV_PORT);

    if (bind(listenfd, (struct sockaddr *)&servaddr, sizeof(servaddr)) == -1)
    {
        perror("bind");
        exit(1);
    }

    if (listen(listenfd, 1024) == -1)
    {
        perror("listen");
        exit(1);
    }

    printf("Listening on port %d\n", SERV_PORT);

    while (1)
    {
        clilen = sizeof(cliaddr);
        connfd = accept(listenfd, (struct sockaddr *)&cliaddr, &clilen);
        if (connfd < 0)
        {
            perror("accept");
            continue;
        }

        pid_t pid = fork();
        if (pid < 0)
        {
            perror("fork");
            close(connfd);
            continue;
        }
        if (pid == 0)
        {
            close(listenfd);
            process_client(connfd);
            close(connfd);
            exit(0);
        }
        else
        {
            close(connfd);
        }
    }
    semctl(semid, 0, IPC_RMID, NULL);
    shmdt(arr);
    shmctl(shmid, IPC_RMID, NULL);
    return 0;
}
